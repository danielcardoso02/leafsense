...
% ----------------------------------------------------------------------------
\subsection{Display \& Touchscreen Configuration}
\label{subsec:display-config}
% ----------------------------------------------------------------------------

The system uses a 3.5" ILI9486 LCD with ADS7846 resistive touchscreen controller connected via SPI. The display configuration required careful calibration to ensure proper touch mapping.

\subsubsection{Boot Configuration}

The display overlay is configured in \texttt{/boot/config.txt}:

\begin{lstlisting}[language=bash, caption={Display Boot Configuration}]
...
\subsubsection{Qt Touchscreen Integration}

Qt5's evdev touchscreen plugin is used instead of tslib to prevent application freezing during touch events. The touch coordinate transformation is configured via environment variables:

\begin{lstlisting}[language=bash, caption={Qt Touchscreen Environment Configuration}]
# Display on SPI LCD (fb1)
export QT_QPA_PLATFORM=linuxfb:fb=/dev/fb1:size=480x320

# Touchscreen with rotation matching display overlay
export QT_QPA_EVDEV_TOUCHSCREEN_PARAMETERS="/dev/input/event0:rotate=180:invertx"

# Disable libinput to prevent conflicts
export QT_QPA_FB_NO_LIBINPUT=1
export QT_QPA_FB_HIDECURSOR=1
\end{lstlisting}

\textbf{Technical Note:} The touch rotation parameter (\texttt{rotate=180:invertx}) must be calibrated to match the display overlay rotation (\texttt{rotate=270}). Using tslib caused SPI bus contention resulting in application freezes.

\subsubsection{Startup Script}

The application startup script (\texttt{/opt/leafsense/start.sh}) configures the environment and launches LeafSense:

\begin{lstlisting}[language=bash, caption={LeafSense Startup Script}]
#!/bin/sh
cd /opt/leafsense
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|c|}
        ...
        3.5" LCD display (fb1) & \checkmark Working \\
        Touchscreen calibration (evdev) & \checkmark Calibrated \\
        Touch input responsiveness & \checkmark Functional \\
        ML predictions saved to database & \checkmark Working \\
        Analytics window data display & \checkmark Working \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|p{4cm}|p{4.5cm}|p{4.5cm}|}
        ...
        \hline
        DB tables do not exist & DB not initialized & Execute \texttt{sqlite3 leafsense.db < database/schema.sql} \\
        \hline
        ...
        \hline
        Application freezes on touch & tslib SPI bus contention & Use Qt evdev plugin instead of tslib \\
        \hline
        Touch coordinates inverted & Display rotation mismatch & Use \texttt{rotate=180:invertx} in evdev parameters \\
        \hline
        ML predictions not saved & Missing database message handler & Added PRED message handler in dDatabase.cpp \\
        \hline
    \end{tabular}
\end{table}
