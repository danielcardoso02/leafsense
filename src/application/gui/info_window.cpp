...
void InfoWindow::setup_ui()
{
    ...
    QFont t_font = system_title->font(); t_font.setPointSize(11); t_font.setBold(true);
    system_title->setFont(t_font);
    main_layout->addWidget(system_title);

    QVBoxLayout *system_info = new QVBoxLayout();
    system_info->setSpacing(6); system_info->setContentsMargins(10, 0, 10, 0);
    QFont info_font; info_font.setPointSize(9);

    QLabel *app_label = new QLabel("Application: LeafSense v1.0.0"); app_label->setFont(info_font);
    system_info->addWidget(app_label);

    QLabel *creators_label = new QLabel("Developers: Daniel Cardoso and Marco Costa"); creators_label->setFont(info_font);
    ...
    user_title->setFont(t_font);
    main_layout->addWidget(user_title);

    QVBoxLayout *user_info = new QVBoxLayout();
    user_info->setSpacing(6); user_info->setContentsMargins(10, 0, 10, 0);

    QLabel *username_label = new QLabel("Username: " + username); username_label->setFont(info_font);
    user_info->addWidget(username_label);

    QLabel *login_label = new QLabel("Login Time: " + login_time); login_label->setFont(info_font);
    user_info->addWidget(login_label);

    QLabel *current_time_label = new QLabel("Current Time: " + QDateTime::currentDateTimeUtc().toString("yyyy-MM-dd HH:mm:ss UTC"));
    ...
    // --- CLOSE BUTTON (Fixed Uniform Size) ---
    QHBoxLayout *button_layout = new QHBoxLayout();
    button_layout->addStretch(); // Center

    QPushButton *close_btn = new QPushButton("Close");
    close_btn->setFixedSize(100, 32); // Uniform Size
